...
import { buildPlayers } from "./playersHelper";
...
const getPlayersFormation = (squad) => {
  let DEFs = squad.filter(
    (p) => !p.isSubstitutePlayer && p.position === POSITION_DEF
  ).length;
  let MIDs = squad.filter(
    (p) => !p.isSubstitutePlayer && p.position === POSITION_MID
  ).length;
  let FWDs = squad.filter(
    (p) => !p.isSubstitutePlayer && p.position === POSITION_FWD
  ).length;
...
const setPlayersAdditionalData = (teamData) => {
  const team = buildPlayers(teamData.squad);

  const squad = clone(team).map((player) => {
...
const handlesP11Click = ({ player, squadInfo }) => {
  ...
  const swapOut = readyPlayerBeforeSwapping(
    squad[swapInIndex],
    squad[swapOutIndex]
  );
  const swapIn = readyPlayerBeforeSwapping(
    squad[swapOutIndex],
    squad[swapInIndex]
  );
...
const disablePlayer = ({ p, index, player, squadInfo }) => {
  ...
  return (
    ...
    (formationInfo.def === 3 &&
      !p.isSubstitutePlayer &&
      p.position === POSITION_DEF) ||
    // Disable MIDs if Just 3 Left
    (formationInfo.mid === 3 &&
      !p.isSubstitutePlayer &&
      p.position === POSITION_MID) ||
    // Disable FWDs if Just 1 Left
    (formationInfo.fwd === 1 &&
      !p.isSubstitutePlayer &&
      p.position === POSITION_FWD)
  );
};

const handlesSubstituteClick = ({ player, arrayIndex, squadInfo }) => {
  const squad = clone(squadInfo.squad);
  ...
  const $squad = squad.map((p, index) => {
    ...
    if (disablePlayer({ p, index, player, squadInfo })) {
      ...
      // Attach Icons & other data to clicked substitute and relevant p11
      if (p.id === player.id) {
        p.clickedIcon = DIAMOND_UP_GREEN;
        p.latestSubToBeSwappedIn = true;
      } else {
...
export const playerSwapHandler = (props) => {
  const { player, setTransferInProgress } = props;
...
export const makeCaptain = ({ squadInfo, player, captainType }) => {
  ...
  const previousCaptain = readyCaptainBeforeChange(
    squad[pIndex],
    squad[nIndex]
  );
