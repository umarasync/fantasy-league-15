...
export default function Input({
  ...
  return (
    <div
      ...
      style={{ ...STYLES.parentContainerStyle, ...parentContainerStyle }}
    >
      {!hideLabel && <Label />}
      ...
      {dropDownOfInlineStyle ? (
        ...
        >
          ...
          <div className={"flex items-center"} style={STYLES.arrowImage}>
            <img
              src={opened ? "/images/arrow_up.png" : "/images/arrow_down.png"}
              width={R(24)}
              height={R(24)}
      ...
      {opened ? (
        <AnimatePresence>
          <motion.div
            className={`absolute z-[100] border-[1px] rounded-[1.2rem] shadow-[4px 4px 40px rgba(0, 0, 0, 0.03)] bg-white ${
              !dropDownOfInlineStyle && "w-full"
            }`}
            variants={Animation}
            initial="initial"
            animate="animate"
          ...
          >
            {options.map((option, index) => {
              return !index && skipFirstOption ? null : (
                <div
                  key={index}
                ...
                >
                  <p
                    className={`font-[600] hover:text-mandy text-black_rock rounded-t-[1.2rem]`}
                    style={{
                      ...STYLES.optionText,
                    }}
                  >
