...
const getStyles = (R, props) => {
  ...
  return {
    container: {
      ...
      background: selected
        ? "linear-gradient(180deg, #EE6384 0%, #D9335B 100%)"
        : colors.white,
...
export default function PlayerCard(props) {
  ...
  return (
    <div
      className={"card-box flex items-center justify-between"}
    ...
    >
      ...
      <div className={"flex items-center"}>
        <div style={STYLES.infoImage}>
          <img
            src={`/images/${selected ? "info_light.png" : "info_grey.png"}`}
            width={"100%"}
            height={"100%"}
            alt=""
          />
      ...
      <div className={"flex items-center"}>
        <div style={{ marginRight: R(16) }}>
          <p style={STYLES.price}>
            ...
            <p style={STYLES.type}>
              <span>{positionAbbr(player.position)}</span>
            </p>
          </p>
        </div>
        <Border />
        <p style={STYLES.totalPoints}>{player.totalPoints}</p>
      </div>
    </div>
  );
}
