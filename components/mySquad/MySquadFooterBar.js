...
export default function MySquadFooterBar({
  ...
  // Global States
  const boosters = useSelector(({ auth }) => auth.user.boosters);
  const { benchBoostApplied, tripleCaptainApplied } = boosters;
  const loadingFantasyTeamSwapping = useSelector(
    ({ fantasyTeam }) => fantasyTeam.loadingFantasyTeamSwapping
  );
  ...
  const handleBenchBoostModal = () => {
    const substitutePlayer = squadInfo.squad.filter(
      (p) => p.isSubstitutePlayer
    );
